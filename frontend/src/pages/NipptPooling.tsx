...
const AMOUNT_MAP:Record<string,number>={DBS:250,BLOODSTAIN:250,HAIR:300,NAIL:160,SWAB:120,TOOTHBRUSH:450,CIGARETTE:120,BEARD:230};
...
interface BatchItem { id:string; batch_number:string; status:string; status_display:string; sample_count:number; female_count:number; male_blood_count:number; male_other_count:number; receipt_location?:string; }
...
const [reviewers, setReviewers] = useState<Record<string,string>>({});
  ...
  const groups = useMemo(():PoolGroup[]=>{
    ...
    for (let g = 0; g < numGroups; g++) {
      ...
      const dispRows = selectedBatch?.status === "COMPLETED" ? groupRows : sortBySexThenPt(groupRows);
  ...
  /** 回车跳本列下一格（Mix 表：浓度/洗脱/投入；跨组自动衔接，到底停住） */
  const focusNextPoolCell = (ri:number, field:string) => {
    const el = document.querySelector(`[data-pool-cell="${ri+1}_${field}"]`) as HTMLElement|null;
    if (!el) return;
  ...
  const savePersons = async (): Promise<boolean> => {
    ...
    try {
      await api.patch("/cases/pooling/" + selectedBatch.id + "/", {
              ...
              <div key={gi} style={{marginBottom:24}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,flexWrap:"wrap",gap:8}}>
                  ...
                  <Space size={2} style={{flexWrap:"wrap"}}>
                    ...
                    <Button size="small" style={{fontSize:11,padding:"0 4px"}} disabled={hkDoubled} onClick={()=>{if(hkDoubled)return;const keys:Record<string,number>={FEMALE_BLOOD:(customAmounts["FEMALE_BLOOD"]??200)*2,MALE_BLOOD:(customAmounts["MALE_BLOOD"]??120)*2,BLOODSTAIN:(customAmounts["BLOODSTAIN"]??250)*2,DBS:(customAmounts["BLOODSTAIN"]??250)*2,HAIR:(customAmounts["HAIR"]??300)*2,NAIL:(customAmounts["NAIL"]??160)*2,TOOTHBRUSH:(customAmounts["TOOTHBRUSH"]??450)*2,CIGARETTE:(customAmounts["CIGARETTE"]??120)*2,BEARD:(customAmounts["BEARD"]??230)*2};setCustomAmounts(p=>({...p,...keys}));setRows(prev=>prev.map(r=>{const k=r.category==="FEMALE_BLOOD"?"FEMALE_BLOOD":r.category==="MALE_BLOOD"?"MALE_BLOOD":r.sampleType;const val=keys[k as string];if(val===undefined)return r;return{...r,poolingAmount:val,poolingVolume:(r.concentration??0)>0?Math.round(val/(r.concentration??1)*100)/100:0}}));setHkDoubled(true);}}>香港翻倍</Button>
  ...
  return (<div key={e.case_sample_ids.join(",")} style={{padding:"4px 8px",borderBottom:"1px solid #f0f0f0",display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <Text code style={{fontSize:11,width:150,flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.case_number}</Text><div style={{width:104,flexShrink:0}}>{e.test_sample_id&&<Tag color="blue" style={{fontSize:11,maxWidth:104,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",verticalAlign:"middle"}}>{e.test_sample_id}</Tag>}</div><Tooltip title={e.patient_name}><div style={{flex:1,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:600}}>{e.patient_name}</div></Tooltip>{e.receipt_location&&<Tag color={e.receipt_location==="HONGKONG"?"geekblue":"volcano"} style={{fontSize:10,marginRight:0,flexShrink:0}}>📍{LOC_LABEL[e.receipt_location]||e.receipt_location}</Tag>}<Tooltip title={e.expected_completion||""}><div style={{width:84,flexShrink:0,textAlign:"right",fontSize:11,color:"rgba(0,0,0,0.45)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.expected_completion||""}</div></Tooltip></div>)})}</div>)
            })}
          </div>
        </div>)}
      </Modal>
