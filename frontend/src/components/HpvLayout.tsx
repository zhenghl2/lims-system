...
import {
  DashboardOutlined, ExperimentOutlined, InboxOutlined,
...
const MENU_ITEMS = [
  { key: "/hpv/dashboard",   icon: <DashboardOutlined />,   label: "Dashboard" },
  { key: "/hpv/samples",     icon: <ExperimentOutlined />,   label: "Sample Registration" },
  { key: "/hpv/receiving",   icon: <InboxOutlined />,        label: "Sample Receiving" },
  { key: "/hpv/workflow",    icon: <ApartmentOutlined />,    label: "Lab Workflow" },
  { key: "/hpv/reports",     icon: <FileTextOutlined />,     label: "Reports" },
];
...
export default function HpvLayout({ children, header }: Props) {
  ...
  const selectedKey = MENU_ITEMS.find(item => location.pathname.startsWith(item.key))?.key || "/hpv/dashboard";
      ...
      <Layout>
        <Header style={{
          ...
          <Space size="middle">
            ...
            <Text strong style={{ fontSize: 14 }}>{header || "HPV Subsystem"}</Text>
          </Space>

          <Space size="middle">
            {(!user?.allowed_panels || user.allowed_panels.length === 0 || user.allowed_panels.length > 1 || user.allowed_panels[0] !== "HPV") && (
        ...
        <Footer style={{ textAlign: "center", color: token.colorTextDescription, padding: "12px 50px" }}>
          HPV LIMS &copy; {new Date().getFullYear()}
        </Footer>
      </Layout>
    </Layout>
  );
