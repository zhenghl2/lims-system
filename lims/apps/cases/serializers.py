...
class NipptImportFatherSerializer(serializers.Serializer):
    """巴西导入：单个疑父"""
    name = serializers.CharField(allow_blank=True, default="")
    id_card = serializers.CharField(required=False, allow_blank=True, default="")
...
class NipptImportCaseItemSerializer(serializers.Serializer):
    """巴西导入：一个 Case（Seq 基号分组后）"""
    seq = serializers.CharField()
    mother_name = serializers.CharField()
    mother_dob = serializers.CharField(required=False, allow_blank=True, default="")      # DD/MM/YYYY
    ...
    balance = serializers.CharField(required=False, allow_blank=True, default="")
...
class NipptExtractionBatchDetailSerializer(serializers.ModelSerializer):
    ...
    def get_female_samples(self, obj):
        qs = obj.samples.filter(category="FEMALE_BLOOD")
        # 仅未完成批次按 PT 编号排序；已完成批次保持数据库自然顺序（历史孔板图不变）
        if obj.status != "COMPLETED":
            qs = qs.order_by(Length("case__pt_number"), "case__pt_number", "patient_name")
        return NipptExtractionSampleSerializer(qs, many=True).data
    def get_male_blood_samples(self, obj):
        qs = obj.samples.filter(category="MALE_BLOOD")
        # 仅未完成批次按 PT 编号排序；已完成批次保持数据库自然顺序（历史孔板图不变）
        if obj.status != "COMPLETED":
...
class NipptHybSeqBatchDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = NipptHybSeqBatch
        fields = ["id","batch_number","status","status_display","sample_count","female_count","male_blood_count","male_other_count","female_samples","male_blood_samples","male_other_samples","mix_sources","hyb_seq_data","created_by", "operator_name", "reviewer","created_at","updated_at"]
        read_only_fields = ["id","batch_number","created_at","updated_at"]
    def get_sample_count(self,obj): return obj.samples.count()
    def get_female_count(self,obj): return obj.samples.filter(category="FEMALE_BLOOD").count()
    def get_male_blood_count(self,obj): return obj.samples.filter(category="MALE_BLOOD").count()
