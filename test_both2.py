...
data = json.dumps({"mix_ids": ids, "chip_number": "T3"}).encode()
...
print(f"Both mixes: F={d['female_count']}/{total_f} M={d['male_blood_count']+d['male_other_count']}/{total_m} {'OK' if ok else 'FAIL'}")
req = urllib.request.Request("http://localhost:8000/api/v1/cases/hybseq/pending_mixes/", headers={"Authorization": f"Bearer {token}"})
mixes2 = json.loads(urllib.request.urlopen(req).read()).get("mixes",[])
print(f"Pending after: {len(mixes2)} (should be 0)")
